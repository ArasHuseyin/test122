# test122 — Product Reviews (Judge.me variant)

> Hand-built Shopify app, delivered ready-to-iterate. Live at https://test122.appapprove.app
>
> **Project facts:**
> - Blueprint: Product Reviews
> - Variant: **Judge.me** (default) — Like Judge.me — photo reviews, Q&A, verified buyer badges.
...
A Judge.me-class Product Reviews app: collect star ratings + written reviews + photos from your customers, display them via a theme-app-embed block, moderate from the embedded admin. Includes Q&A per product, verified-buyer badges (auto-detected), and a 2-stage review-request email pipeline (initial after fulfillment + reminder 14d later).

**Shipped surfaces:**
- Star ratings + written reviews + optional photo uploads
- Theme-app-embed block (no theme code edits)
- Q&A per product (customer asks, merchant answers)
- Verified-buyer badges auto-detected from order history
- Embedded admin: approve / reject / reply / search
- 2-stage email pipeline: request 7d post-fulfillment + 14d reminder
...
- *"add a rating-distribution bar chart to the admin index"*
- *"change the storefront widget star color to my brand green"*
- *"add CSV export of all reviews to the admin"*
- *"add a photo lightbox to the storefront review gallery"*
- *"send a weekly summary email to the merchant on Mondays"*

## Environment variables

This app is hosted by AppApprove. Environment variables and secrets are managed by the project owner in the AppApprove dashboard (**Settings → Environment**) and injected into the Worker by the deploy pipeline on the next push.

When you add code that reads a new `env.X` variable, do NOT instruct the user to edit `.env`, edit `wrangler.toml`, or run `wrangler secret put` — they have no direct Worker access. Instead, tell them the exact variable name(s) to add under Settings → Environment.

This app expects these to be set:

- **`RESEND_API_KEY`** — Resend API key. For review-request + reminder emails. Get one at resend.com.
- **`RESEND_FROM`** — Sender email address. Domain must be verified in Resend (e.g. reviews@yourshop.com).
...
- MANDATE: the App Bridge CDN script (`<script src="https://cdn.shopify.com/shopifycloud/app-bridge.js" />`) MUST live in root.tsx `<head>`, MUST be the first `<script>` tag, and MUST NOT use `defer`, `async`, or `type="module"`. Loading it from `<body>` or with `defer` aborts App Bridge initialization with a console error and leaves `window.shopify` undefined.
- MANDATE: embedded-admin `<Link>` elements to sibling/child admin routes MUST NOT use `prefetch` (`prefetch="intent"` / `"render"` / `"viewport"`). Remix prefetch fires a speculative loader request that carries neither the `?shop=` query param nor the App Bridge `Authorization: Bearer` session token, so the target route's admin auth throws 401 — `Failed to load resource: 401` console spam on hover. Use the default (no `prefetch` prop) for admin-to-admin navigation. Storefront / non-embedded routes may still use prefetch.
...
- Error reporting (Phase 10 B): every generated `app/entry.server.tsx` MUST export a `handleError(error, { request })` hook that awaits `reportError(loadContext, { error, request })` from `~/lib/error-reporter.server.ts`. The reporter posts to AppApprove's `/api/v1/error-report` with HMAC signing so runtime errors surface in the customer's Monitor tab. The helper is generated automatically — call it; do not re-implement HMAC or hard-code the project ID.
...
→ `deploy.yml` runs install + tsc + test + `wrangler deploy`. Live URL is `test122.appapprove.app`.
